...
def join_sequences(frequent_items, maxspan, maxgap):
    ...
    # print(check_candidate_gen([OrderedSet(['C','W']),OrderedSet(['R'])],[OrderedSet(['W']),OrderedSet(['C','R'])]))
    # print(check_candidate_gen([OrderedSet(['W']),OrderedSet(['R','W'])],[OrderedSet(['C','R','W'])]))
    # print(check_candidate_gen([OrderedSet(['C','W']),OrderedSet(['R'])],[OrderedSet(['W','R','S'])]))
    # print(check_candidate_gen([OrderedSet(['W']),OrderedSet(['C','R']),OrderedSet(['S'])],[OrderedSet(['C','R']),OrderedSet(['S']),OrderedSet(['S'])]))
    
    for i in range(len(frequent_items)):
        Sa = copy.deepcopy(frequent_items[i])
 
        for j in range(len(frequent_items)):

            Sb = copy.deepcopy(frequent_items[j])

            Sc = []

            if check_candidate_gen(Sa,Sb) != 0:

                if length(Sa[0])==1 and length(Sb[0])==1 and Sa[0]!=Sb[0]:
                    if Sa[0].items[0]<Sb[0].items[0]:
                        s = [Sa[0].items[0],Sb[0].items[0]]
                        candidates.append([OrderedSet([''.join(s)])])

                # If the first itemset of Sa is a singleton, Sc is generated by prepending it to Sb
                if length(Sa[0])==1: # Sa[0] is a singleton
                    Sc = list([OrderedSet([''])]*(size(Sb)+1)) # initialization to size Sb+1
                    Sc[0] = Sa[0]
                    if length(Sb[-1])==1: # the last itemset of Sb is a singleton, Sc is generated by appending it to Sa
                        Sc[1:] = Sa[1:]
                        Sc.append(Sb[-1])
                    else: # Sb[-1] is not a singleton, Sc is generated by replacing the last itemset of Sa with it
                        Sc[1:] = Sa[1:]
                        Sc.append(Sb[-1])
                ...
                # If the first itemset of Sa is not a singleton, Sc is generated by replacing the first itemset of Sb with it
                    ...
                    Sc = list([OrderedSet([''])]*(size(Sb))) # initialization to size Sb
                    Sc = Sb
                    Sc[0] = Sa[0]
                    if(length(Sb[-1])==1):    # Sb[-1] is a singleton, Sc is generated by appending it to Sa
                        Sc.append(Sb[-1])
                    else: #  Sb is not a singleton, Sc is generated by replacing the last itemset of Sa with it
                        Sc[-1] = Sb[-1]
                    candidates.append(Sc)
...
def length(S): # length of a list of itemsets
    l = 0
    for itemset in S:
        l += len(itemset)
    return l
...
def check_candidate_gen(Sa, Sb): 
    ...
    Sa1 = copy.deepcopy(Sa)
    Sb1 = copy.deepcopy(Sb)

    if len(Sa1[0])==1:
        Sa1 = Sa1[1:] # If the itemset is a singleton, the entire itemset is deleted for the test:
        if len(Sb1[-1])==1: # Sa[0] is a singleton, Sb[-1] is a singleton
            Sb1 = Sb1[0:len(Sb1)-1] 
            return check(Sa1,Sb1)
        else: # Sa[0] is a singleton, Sb[-1] is not
            Sb1[-1].pop(0)
            return check(Sa1,Sb1)
    else: # Sa[0] is not a singleton
        Sa1[0].pop(0)
        if len(Sb1[-1])==1: # Sa[0] is not a singleton, Sb[-1] is a singleton
            Sb1 = Sb1[0:len(Sb1)-1]
            return check(Sa1,Sb1)
...
def find(dataset,candidate,max_gap): # find the sequence in the dataset
    ...
    for i in range(1,len(dataset)): # the first element is the first candidate that we already identified
        for j in range(len(candidate)):
            if is_contained(candidate[j],dataset[i]) and gap <= max_gap:
                flag[j] = 1
...
def is_contained(element,sequence):
    # example: if ['C'] is contained in ['CW']
    candidate_set = set(list(element))
    itemset_set = set(list(sequence))
    return candidate_set.issubset(itemset_set)


def count_support_aux(candidate,dataset,maxspan,maxgap):
    ...
    while dataset != []:
        ...
        # search the first occurrence of candidate[0]
        for i in range(len(dataset)):
            if is_contained(candidate[0],dataset[i]):
                dataset = dataset[i:] # cut the dataset: the first element is now the itemset == candidate[0]
                break
        ...
        if len(candidate)==1:
            ...
            k += find(dataset[:maxspan],candidate[1:],maxgap)
            # returns 1 if it founds all the other candidates in the dataset (considering the max_span)
...
def count_support(dataset, candidates, maxspan, maxgap):
    ...
    for candidate in candidates:
        k = count_support_aux(candidate, dataset, maxspan, maxgap)
        if k:
            # itemset = "".join(itemset[0])
            key = '-'.join(sorted(''.join(s) for s in candidate))
...
def count_frequent_items(dataset):
    ...
    for item in item_counts:
        if item_counts[item] >= minsup:
            result.update({item: {'subsequence': [OrderedSet(item)], 'support': item_counts[item]}})
     # item: count for item, count in item_counts.items() if count >= minsup}

    return result

def print_os(list_orderedset):
    k = 1
    for orderedset in list_orderedset:
        listed = {}
        string = []
        for s in orderedset:
            string.append(''.join(s))
        listed[k] = {'c': string, 'size': size(string), 'length': length(string)}
        print(f"Candidate {k}: {listed[k]['c']}, Size: {listed[k]['size']}, Length: {listed[k]['length']}")
        k +=1
...
def gsp(dataset, minsup, maxspan, maxgap):
    ...
    # TEST JOIN SEQUENCES
    # trial = [[OrderedSet(['C','W']),OrderedSet(['R'])],[OrderedSet(['W']),OrderedSet(['C','R'])]]
    # print_os(join_sequences(trial,maxspan,maxgap))
...
dataset = ['W','S','W','S','W','C','CR','C','CRW','CW','CRW','CW','W','S','C','S','W','CW','CRW','CR']
...
for key in result:
    print(f"Sequence: {key}, Support: {result[key]['support']}")
